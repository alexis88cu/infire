...
export default function ProjectPage({params}:{params:{slug:string}}) {
  ...
  return (
    <div style={{maxWidth:'900px',margin:'0 auto',padding:'4rem 2rem'}}>
      ...
      <div style={{height:'360px',background:'linear-gradient(135deg,#1a2332 0%,#0d1520 100%)',borderRadius:'12px',overflow:'hidden',marginBottom:'2.5rem',position:'relative',border:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'center'}}>
        {p.featuredImage ? (
          <img src={p.featuredImage} alt={p.projectName} style={{width:'100%',height:'100%',objectFit:'cover'}} />
        ) : (
        ...
        <div style={{position:'absolute',top:16,left:16,display:'flex',gap:'8px'}}>
          ...
          <span className="badge badge-gray" style={{fontSize:'0.75rem',padding:'4px 10px',background:'rgba(13,17,23,0.85)'}}>{p.projectType}</span>
        </div>
      </div>
      ...
      {tags.length>0 && (
        <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',marginBottom:'3rem'}}>
          {tags.map((tag:string)=><span key={tag} className="tag-orange">{tag}</span>)}
        </div>
      )}
